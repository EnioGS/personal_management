...
This is a *web* app — it will run as a hosted service rather than be
distributed as a binary. A permissive or plain-GPL
...
License the project under GNU AGPL-3.0. AGPL closes the network-service
gap: anyone who lets users interact with a modified version over a network
must also make that version's source available.

## Consequences

Anyone can use and modify this project, but a modified version must stay
open source even when it's only ever run as a service and never
"distributed" in the traditional sense. A meaningfully more restrictive
choice than the MIT/permissive licenses commonly used for similar projects —
