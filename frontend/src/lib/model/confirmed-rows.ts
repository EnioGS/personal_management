...
export const CONFIRMED_EDITABLE = ['date', 'value', 'amount', 'class', 'category', 'subcategory', 'account', 'card', 'observations'] as const
...
export async function updateConfirmedRow(rowId: number, column: ConfirmedEditableColumn, value: string): Promise<void> {
  ...
  const patch: Partial<ConfirmedRow> =
    ...
    : column === 'value' || column === 'amount' ? { [column]: parseNumberValue(value) ?? undefined }
